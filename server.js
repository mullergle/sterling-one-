...
async function cleanupRegistration(userId, accountId = null) {
  if (!userId) return;
  ...
  try {
    if (accountId) {
      ...
      if (error) console.error("CLEANUP BALANCE ERROR:", error);
    }
  } catch (error) {
    console.error("CLEANUP BALANCE EXCEPTION:", error);
  }
  ...
  try {
    if (accountId) {
      const { error } = await supabase
        ...
        .eq("id", accountId)
        .eq("user_id", userId);
      if (error) console.error("CLEANUP ACCOUNT ERROR:", error);
    }
  } catch (error) {
    console.error("CLEANUP ACCOUNT EXCEPTION:", error);
  }
  ...
  try {
    const { error } = await supabase
      ...
      .delete()
      .eq("user_id", userId);
    if (error) console.error("CLEANUP ADDRESS ERROR:", error);
  } catch (error) {
    console.error("CLEANUP ADDRESS EXCEPTION:", error);
  }
  ...
  try {
    const { error } = await supabase
      ...
      .delete()
      .eq("user_id", userId);
    if (error) console.error("CLEANUP CONSENT ERROR:", error);
  } catch (error) {
    console.error("CLEANUP CONSENT EXCEPTION:", error);
  }
  ...
  try {
    const { error } = await supabase
      ...
      .delete()
      .eq("user_id", userId);
    if (error) console.error("CLEANUP NOTIFICATION ERROR:", error);
  } catch (error) {
    console.error("CLEANUP NOTIFICATION EXCEPTION:", error);
  }
  ...
  try {
    const { error } = await supabase
      ...
      .delete()
      .eq("id", userId);
    if (error) console.error("CLEANUP PROFILE ERROR:", error);
  } catch (error) {
    console.error("CLEANUP PROFILE EXCEPTION:", error);
  }

  try {
    const { error } = await supabase.auth.admin.deleteUser(userId);
    if (error) console.error("CLEANUP AUTH USER ERROR:", error);
  } catch (error) {
    console.error("CLEANUP AUTH USER EXCEPTION:", error);
  }
}
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (accountError) {
        console.error("ACCOUNT STATUS UPDATE ERROR:", accountError);
        return res.status(500).json({
          success: false,
...
app.post("/support/messages", authenticate, async (req, res) => {
  try {
    ...
    if (conversationError) {
      ...
      return res.status(500).json({
        success: false,
        message: "Unable to find support conversation",
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      if (profilesError) {
        console.error("ADMIN USERS PROFILE ERROR:", profilesError);
        return res.status(500).json({
          success: false,
      ...
      try {
        ...
        console.error("ADMIN AUTH USERS EXCEPTION:", error);
      }

      const authMap = new Map();
      authUsers.forEach((user) => authMap.set(user.id, user));

      const accountsByUser = new Map();
      (accounts || []).forEach((account) => {
        if (!account.user_id) return;
        if (!accountsByUser.has(account.user_id)) {
          accountsByUser.set(account.user_id, []);
        }
      ...
      const users = (profiles || [])
        .filter((profile) => !isAdminValue(profile.is_admin))
        .map((profile) => {
          const authUser = authMap.get(profile.id);
          const userAccounts = accountsByUser.get(profile.id) || [];
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        ...
        return res.status(500).json({
          success: false,
          message: "Unable to load support messages: " + error.message,
          error: error.message,
...
app.post(
  ["/admin/support/messages", "/api/admin/support/messages"],
  authenticateAdmin,
  async (req, res) => {
    try {
      const { user_id, message } = req.body || {};
      ...
      if (!existingMessage?.conversation_id) {
        return res.status(404).json({
          success: false,
          message: "No support conversation exists for this customer"
        });
      }
      ...
      try {
        ...
        console.error("SUPPORT NOTIFICATION EXCEPTION:", error);
      }
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      console.error("ADMIN SUPPORT READ EXCEPTION:", error);
      return res.status(500).json({
        success: false,
        message: "Unable to mark messages as read"
      });
...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    const cleanSurname = String(sname || "").trim();
    ...
    if (profileError) {
      ...
      await cleanupRegistration(createdUserId);
      return res.status(500).json({
        success: false,
    ...
    try {
      ...
      if (error) console.error("TERMS CONSENT ERROR:", error);
    } catch (error) {
      console.error("TERMS CONSENT EXCEPTION:", error);
    }
    ...
    try {
      ...
      if (error) console.error("PRIVACY CONSENT ERROR:", error);
    } catch (error) {
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const { data: profile, error: profileError } = await supabase
      .from("profiles")
      .select("id, first_name, surname, phone, is_admin")
      .eq("id", user.id)
      .maybeSingle();
...
app.get("/api/auth/me", authenticate, async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("ME ADDRESS EXCEPTION:", error);
    }
...
app.get("/api/accounts/:id", authenticate, async (req, res) => {
  try {
    ...
    return res.json({
      ...
      account: {
        ...account,
        account_balances: balance ? [balance] : []
      }
    });
  } catch (error) {
    console.error(error);
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        account_identifier: String(account_identifier).trim(),
...
app.delete("/api/beneficiaries/:id", authenticate, async (req, res) => {
  try {
    const { error } = await supabase
      .from("beneficiaries")
      .delete()
...
app.post("/api/transfers", authenticate, async (req, res) => {
  try {
    ...
    if (!recipientName || !recipientBank || !swiftCode) {
      return res.status(400).json({
        success: false,
        message: "Recipient name, bank and SWIFT code are required"
      });
    }
    ...
    return res.status(201).json({
      success: true,
      message: "Transfer verification required",
      transfer: {
        id: transfer.id,
...
app.post("/api/transfers/verify", authenticate, async (req, res) => {
  try {
    const { transfer_id, code } = req.body || {};

    if (!transfer_id || !code) {
      return res.status(400).json({
        success: false,
        message: "Transfer ID and verification code are required"
      });
    }
    ...
    if (transfer.status !== "pending") {
      return res.status(400).json({
        success: false,
        message: "This transfer is no longer awaiting verification"
      });
    }
    ...
    const currentLedger = Number(senderBalance.ledger_balance);
    const debitAmount = Number(transfer.amount);
  ...
  async (req, res) => {
    try {
      ...
      return res.json({
        ...
        balances: { checking, savings, card }
      });
    } catch (error) {
